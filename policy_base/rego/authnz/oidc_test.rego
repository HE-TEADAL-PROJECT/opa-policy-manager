...
test_preferred_token_jwks_url_1 {
	...
	url_lookup := {"http://localhost": want}
...
test_preferred_token_jwks_url_2 {
	...
	url_lookup := {"http://localhost:8080": want}
	got := preferred_token_jwks_url(token_payload, url_lookup)
	got == want
}
...
test_token_jwks_preferred_url {
	token_payload := {"iss": "http://localhost/keycloak/realms/master"}
	url_lookup := {"http://localhost": "http://localhost/keycloak/realms/master/protocol/openid-connect/certs"}
	jwks := token_jwks(token_payload, url_lookup) with data.authnz.oidc.fetch_token_jwks as jwks_tasty_config
...
test_token_jwks_canonical_url {
	token_payload := {"iss": "http://localhost/keycloak/realms/master"}
	url_lookup := {}
	jwks := token_jwks(token_payload, url_lookup) with data.authnz.oidc.fetch_token_issuer_jwks_url as "mock"
...
test_extract_bearer_token {
	...
	request := {"headers": {"authorization": make_bearer_auth(dummy_jwt)}}
...
test_claims_with_static_jwks {
	config := {"jwks": jwks_tasty_config}
	payload := {
		"iss": "me",
		"exp": 10000000000, # 20 Nov 2286 @ 18:46:40 (CET)
	}
	token := generate_tasty_token(payload)
	request := {"headers": {"authorization": make_bearer_auth(token)}}
	payload == claims(request, config)
}

test_claims_with_dynamic_jwks {
	config := {"jwks_preferred_urls": {"http://mock": "http://mock/jwks"}}
	payload := {
		"iss": "http://mock",
		"exp": 10000000000, # 20 Nov 2286 @ 18:46:40 (CET)
	}
	token := generate_tasty_token(payload)
	request := {"headers": {"authorization": make_bearer_auth(token)}}
	extracted_payload := claims(request, config) with data.authnz.oidc.fetch_token_jwks as jwks_tasty_config
